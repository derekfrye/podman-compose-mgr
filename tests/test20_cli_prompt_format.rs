...
use podman_compose_mgr::read_interactive_input::{do_prompt_formatting, unroll_grammar_into_string};
use walkdir::WalkDir;
...
fn prompt_formatting_widths_match_for_all_items() {
        ...
        if entry.file_name() == "docker-compose.yml" {
                ...
                && let Some(services) = yaml.get("services").and_then(|v| v.as_mapping())
            {
                for (_, svc_cfg) in services {
                    ...
                    items.push((entry.path().to_path_buf(), img.to_string(), container.to_string()));
                }
            }
    ...
    for (path, image, container) in items {
        ...
        assert!(s60.len() <= 60, "prompt at width 60 should be <= 60 chars, got {}: {}", s60.len(), s60);
        ...
        assert!(s40.len() <= 40, "prompt at width 40 should be <= 40 chars, got {}: {}", s40.len(), s40);
        ...
        if image.starts_with("djf/") {
            assert!(s40.contains("d..."), "expected shortened image 'd...' in 40-col prompt: {}", s40);
        } else if image.starts_with("pihole/") {
            assert!(s40.contains("p..."), "expected shortened image 'p...' in 40-col prompt: {}", s40);
        }
    }
