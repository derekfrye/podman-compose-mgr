...
fn test_upload_process_with_varying_terminal_sizes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let args = Args {
        ...
        azure_client_id_path: Some(std::path::PathBuf::from("tests/test3_and_test4/a")),
        azure_client_secret_path: Some(client_secret_path),
        azure_tenant_id_path: Some(std::path::PathBuf::from("tests/test3_and_test4/b")),
        azure_vault_name_path: Some(std::path::PathBuf::from("tests/test3_and_test4/c")),
        verbose: 1,
