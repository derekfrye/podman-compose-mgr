...
fn test_initialize_process() {
    ...
    for entry in &output_json {
        ...
        if destination_cloud == "b2" || destination_cloud == "r2" {
            assert_eq!(
                cloud_upload_bucket, "bucket_required_during_upload",
    ...
    assert_eq!(
        output_json.len(),
        6,
        "Expected 6 entries after second run (5 updated + 1 new)"
    );
    ...
    for entry in &output_json {
        ...
        if filenm != "tests/test3_and_test4/f" && first_run_timestamps.contains_key(&filenm) {
            ...
            assert_ne!(
                first_run_ts, second_run_ts,
                "Timestamp for {} should have been updated but wasn't",
                filenm
            );
        }
    }
    ...
    assert!(
        file_seen.contains("tests/test3_and_test4/a"),
        "Missing file 'a'"
    );
    assert!(
        file_seen.contains("tests/test3_and_test4/b"),
        "Missing file 'b'"
    );
    assert!(
        file_seen.contains("tests/test3_and_test4/c"),
        "Missing file 'c'"
    );
    assert!(
        file_seen.contains("tests/test3_and_test4/d d"),
        "Missing file 'd d'"
    );
    assert!(
        file_seen.contains("tests/test3_and_test4/e e"),
        "Missing file 'e e'"
    );
    assert!(
        file_seen.contains("tests/test3_and_test4/f"),
        "Missing new file 'f'"
    );
    ...
    assert_eq!(
        a_entry["hash_algo"].as_str().unwrap(),
        "sha1",
        "Hash algorithm should be SHA-1"
    );
    assert_eq!(
        a_entry["encoding"].as_str().unwrap(),
        "utf8",
        "File 'a' should be utf8 encoded"
    );
    assert_eq!(
        a_entry["cloud_id"].as_str().unwrap(),
        "",
        "cloud_id should be empty"
    );
    assert_eq!(
        a_entry["cloud_cr_ts"].as_str().unwrap(),
        "",
        "cloud_cr_ts should be empty"
    );
    assert_eq!(
        a_entry["cloud_upd_ts"].as_str().unwrap(),
        "",
        "cloud_upd_ts should be empty"
    );

    // Verify 'b' file
    let b_entry = &entries_by_filename["tests/test3_and_test4/b"];
    assert_eq!(
        b_entry["hash_algo"].as_str().unwrap(),
        "sha1",
        "Hash algorithm should be SHA-1"
    );
    assert_eq!(
        b_entry["encoding"].as_str().unwrap(),
        "utf8",
        "File 'b' should be utf8 encoded"
    );

    // Verify 'c' file
    let c_entry = &entries_by_filename["tests/test3_and_test4/c"];
    assert_eq!(
        c_entry["hash_algo"].as_str().unwrap(),
        "sha1",
        "Hash algorithm should be SHA-1"
    );
    assert_eq!(
        c_entry["encoding"].as_str().unwrap(),
        "utf8",
        "File 'c' should be utf8 encoded"
    );

    // Verify 'd d' file
    let d_entry = &entries_by_filename["tests/test3_and_test4/d d"];
    assert_eq!(
        d_entry["hash_algo"].as_str().unwrap(),
        "sha1",
        "Hash algorithm should be SHA-1"
    );
    assert_eq!(
        d_entry["encoding"].as_str().unwrap(),
        "utf8",
        "File 'd d' should be utf8 encoded"
    );

    // Verify 'e e' file
    let e_entry = &entries_by_filename["tests/test3_and_test4/e e"];
    assert_eq!(
        e_entry["hash_algo"].as_str().unwrap(),
        "sha1",
        "Hash algorithm should be SHA-1"
    );
    assert_eq!(
        e_entry["encoding"].as_str().unwrap(),
        "base64",
        "File 'e e' should be base64 encoded"
    );

    // Verify 'f' file (the new file added in the second run)
    let f_entry = &entries_by_filename["tests/test3_and_test4/f"];
    assert_eq!(
        f_entry["hash_algo"].as_str().unwrap(),
        "sha1",
        "Hash algorithm should be SHA-1"
    );
    assert_eq!(
        f_entry["encoding"].as_str().unwrap(),
        "utf8",
        "File 'f' should be utf8 encoded"
    );
    assert_eq!(
        f_entry["file_size"].as_u64().unwrap(),
        2,
        "File 'f' should be 2 bytes"
    );

    // Verify the new 'f' file has no timestamp in first_run_timestamps
    assert!(
        !first_run_timestamps.contains_key("tests/test3_and_test4/f"),
